...
def ResBlock(name, bottom, dim, stride, block_type=None, bottleneck=False):
    layers = []
    if bottleneck:
        ...
        if block_type == 'no_preact':
            res_bottom = bottom
            # 1x1 conv at shortcut branch
#            layers.append(Conv(name + '_proj', res_bottom, dim, 1, stride, 0))
#            shortcut_top = layers[-1].top[0]
            shortcut_top = bottom
        elif block_type == 'both_preact':
            layers.extend(Act(name + '_pre', bottom))
            ...
            res_bottom = layers[-1].top[0]
        # residual branch: conv1 -> conv1_act -> conv2 -> conv2_act -> conv3
        layers.append(Conv(name + '_conv1', res_bottom, dim, 3, stride, 1))
        layers.extend(Act(name + '_conv1', layers[-1].top[0]))
        layers.append(Conv(name + '_conv2', layers[-1].top[0], dim, 3, 1, 1))
        # elementwise addition
        layers.append(Add(name, [shortcut_top, layers[-1].top[0]]))
    return layers
...
def create_model(depth, n_class):
    model = caffe_pb2.NetParameter()
    model.name = 'ResNet{}_cifar{}'.format(depth, n_class)
    configs = {
        20: [3, 3, 3],
        50: [8, 8, 8],
        110: [18, 18, 18],
    ...
    layers.append(Data('data', ['data', 'label'],
                       'examples/cifar%d/cifar%d_train_lmdb' % (n_class, n_class),
                       64, 'train'))
    layers.append(Data('data', ['data', 'label'],
                       'examples/cifar%d/cifar%d_test_lmdb' % (n_class, n_class),
                       50, 'test'))
    layers.append(Conv('conv_bottom', 'data', 16, 3, 1, 1))
    layers.extend(Act('conv_bottom', layers[-1].top[0]))
    layers.extend(ResLayer('group1', layers[-1].top[0], num[0], 16, 1, 'first'))
    layers.extend(ResLayer('group2', layers[-1].top[0], num[1], 32, 2))
    layers.extend(ResLayer('group3', layers[-1].top[0], num[2], 64, 2))
    layers.extend(Act('act_top', layers[-1].top[0]))
    layers.append(Pool('pool', layers[-1].top[0], 'ave', 8, 1, 0))
    layers.append(Linear('fc', layers[-1].top[0], n_class))
...
def main(args):
    model = create_model(args.depth, args.n_class)
    if args.output is None:
        args.output = osp.join(osp.dirname(__file__),
            'resnet{}_cifar{}_traintest.prototxt'.format(args.depth, args.n_class))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--depth', type=int, default=110,
                        choices=[20, 50, 110, 200]) # 3*N + 2
    parser.add_argument('-o', '--output', type=str)
    parser.add_argument('-n', '--n_class', type=int, default=10,
                        choices=[10, 100])
#    parser.add_argument('')
